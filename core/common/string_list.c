...
void string_list_init(StringList *string_list) {
    ...
    string_list->data = (char**)malloc(sizeof(char*)*STRING_LIST_INITIAL_CAPACITY);
}
...
void string_list_append(StringList *string_list, const char* value) {
    ...
    string_list->data[nextIndex] = (char*)calloc((valueLength + 1), sizeof(char));
...
void string_list_set(StringList *string_list, const int index, const char* value) {
    ...
    const size_t valueLength = strlen(value);
    string_list->data[index] = (char*)calloc(valueLength + 1,sizeof(char));
...
char** string_list_export_data(StringList *string_list) {
    ...
    for(int i = 0; i < size; i++) {
        ...
        result[i] = (char*)calloc(strLength + 1, sizeof(char));
